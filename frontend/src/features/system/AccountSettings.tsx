...
const AccountSettings = ({ defaultTab = 0 }: AccountSettingsProps) => {
  ...
  const accountCategories: AccountCategory[] = (accountSubjectsData || []).map(subject => ({
    ...subject,
  ...
  const handleSaveAccount = async () => {
    ...
    try {
      const accountData: UpdateAccountSubjectRequest = {
        subject_code: editingAccount.code,
        subject_name: editingAccount.name,
        subject_type: editingAccount.description,
        industry_type: editingAccount.industry_type,
      };

      if (editingAccount.id > 0) {
        await updateSubject({ id: editingAccount.id, data: accountData }).unwrap();
        setSnackbar({ open: true, message: '勘定科目を更新しました', severity: 'success' });
      } else {
        await createSubject({
          subject_code: editingAccount.code,
          subject_name: editingAccount.name,
